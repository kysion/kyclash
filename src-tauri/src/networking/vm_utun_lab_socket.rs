//! Fixed Unix-socket launcher for the explicit no-sign VirtualMac utun lab.
//!
//! This is not the production tunnel-broker client. The production XPC and
//! code-signing requirements remain separate and unchanged.
...
pub const VM_UTUN_LAB_SOCKET_PATH: &str = "/var/run/net.kysion.kyclash.vm-utun-lab.sock";

#[derive(Debug, Clone)]
pub struct VmUtunLabSocketLauncher {
    socket_path: PathBuf,
    required_peer_uid: libc::uid_t,
}

impl Default for VmUtunLabSocketLauncher {
    fn default() -> Self {
        Self::new()
    }
}

impl VmUtunLabSocketLauncher {
    #[must_use]
    pub fn new() -> Self {
        Self {
            socket_path: PathBuf::from(VM_UTUN_LAB_SOCKET_PATH),
            required_peer_uid: 0,
        }
    }
...
fn validate_socket_metadata(path: &Path) -> Result<(), NetworkErrorCode> {
    ...
    // The root harness creates the socket in root-owned /var/run, then gives
    // only the console user access to this exact inode. The App refuses a
    // symlink, non-socket, loose permissions, root-owned, or foreign-user
...
fn socket_peer_uid(stream: &UnixStream) -> io::Result<libc::uid_t> {
    ...
    // SAFETY: credentials points to writable xucred storage, length describes
    // that storage exactly, and the UnixStream owns a valid descriptor for the
    ...
    // SAFETY: getsockopt succeeded and reported the complete xucred size.
...
impl SidecarProcessControl for VmUtunLabSocketControl {
    ...
    fn try_wait_status(&mut self) -> io::Result<Option<bool>> {
        let mut byte = 0_u8;
        // SAFETY: monitor owns a valid socket descriptor and byte is one byte
        // of writable storage. MSG_PEEK never consumes protocol bytes while
        // MSG_DONTWAIT makes this a bounded liveness probe.
        let result = unsafe {
            libc::recv(
...
impl StdioSidecarLauncher for VmUtunLabSocketLauncher {
    fn launch(&mut self, generation: u64) -> Result<Box<dyn SidecarProcessControl>, NetworkErrorCode> {
        ...
        Ok(Box::new(VmUtunLabSocketControl {
            generation,
            input: Some(input),
            output: Some(output),
            monitor,
...
mod tests {
    ...
    fn socket_fixture(
        ...
        let path = directory.path().join("vm-utun-lab.sock");
    ...
    #[test]
    fn fixed_launcher_transfers_one_authenticated_stream_and_observes_eof() -> anyhow::Result<()> {
        let (directory, path, server) = socket_fixture(0o600, drop)?;
        let mut launcher = VmUtunLabSocketLauncher::injected(path, unsafe { libc::geteuid() });
        let mut control = launcher
            .launch(7)
            .map_err(|error| anyhow::anyhow!("lab launcher failed: {error:?}"))?;
        assert_eq!(control.generation(), 7);
        assert!(control.take_stdin().is_some());
        assert!(control.take_stdout().is_some());
        server.join().map_err(|_| anyhow::anyhow!("join lab server panic"))??;
        let deadline = Instant::now() + Duration::from_secs(1);
        loop {
            if control.try_wait_status()? == Some(true) {
                break;
            }
    ...
    #[test]
    fn launcher_rejects_loose_socket_permissions_before_connect() -> anyhow::Result<()> {
        let (directory, path, server) = socket_fixture(0o660, |_| {})?;
        let mut launcher = VmUtunLabSocketLauncher::injected(path, unsafe { libc::geteuid() });
        assert_eq!(launcher.launch(1).err(), Some(NetworkErrorCode::SidecarUnavailable));
        // Connect once so the test server can finish without weakening the
        // launcher's rejection assertion.
        let _ = UnixStream::connect(directory.path().join("vm-utun-lab.sock"))?;
        server.join().map_err(|_| anyhow::anyhow!("join lab server panic"))??;
        Ok(())
    }

    #[test]
    fn launcher_rejects_non_root_peer_for_production_fixed_constructor() -> anyhow::Result<()> {
        if unsafe { libc::geteuid() } == 0 {
            return Ok(());
        }
        let (_directory, path, server) = socket_fixture(0o600, |_| {})?;
        let mut launcher = VmUtunLabSocketLauncher {
            socket_path: path,
            required_peer_uid: 0,
        };
