...
impl NetworkProfile {
    pub fn validate(&self) -> Result<(), NetworkErrorCode> {
        ...
        if !valid_id(&self.profile_id) || !valid_id(&self.identity_ref) || !valid_id(&self.site.id) {
        ...
        if self.site.display_name.is_empty()
            ...
            || self.tunnel.peer_public_key.is_empty()
        {
            return Err(NetworkErrorCode::InvalidConfiguration);
        }
        ...
        let configured: HashSet<_> = std::iter::once(self.transports.primary)
            ...
            .collect();
        if self.transports.endpoints.iter().any(|endpoint| {
            !configured.contains(&endpoint.transport)
                || Url::parse(&endpoint.url)
                    .ok()
                    .is_none_or(|url| !endpoint_scheme_matches(endpoint.transport, url.scheme()))
        }) {
            return Err(NetworkErrorCode::InvalidConfiguration);
        }
...
const fn endpoint_scheme_matches(transport: TransportKind, scheme: &str) -> bool {
    match transport {
        TransportKind::Quic => matches!(scheme.as_bytes(), b"https" | b"quic"),
