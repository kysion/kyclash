...
const packages = fs.readdirSync(pkgDir).filter((name) => name.endsWith('.pkg') && name.includes('KyClash_'))
if (packages.length !== 1) throw new Error(`expected exactly one KyClash PKG, found ${packages.length}`)
...
const helperPlist = path.join(app, 'Contents/Library/LaunchDaemons/net.kysion.kyclash.route-helper.plist')
const trust = path.join(app, 'Contents/Resources/resources/kyclash-network-sidecar-aarch64-apple-darwin.trust.json')
...
for (const binary of [sidecar, helper]) {
  ...
  if (identity !== team) throw new Error(`unexpected Team ID for ${binary}: ${identity}`)
}
...
if (digest !== trustDigest) throw new Error('sidecar trust manifest hash mismatch')
if (fs.existsSync(path.join(app, 'Contents/Resources/resources/kyclash-route-helper'))) {
